...
func generate_obj() -> IPMDL:
	...
	var lines = FileAccess.open(model_path, FileAccess.READ)
		...
		while lines.get_position() < lines.get_length():
			...
			match parts[0]:
				...
				"mtllib":
					...
					# This is getting too big, it should be moved to it's own function maybe?
					for mtl_line in mtllib:
						...
						match mtl[0]:
							...
							"map_refl", "map_Pm":
								...
								var metalness_texture = ImageTexture.new().create_from_image(metalness_image)
							...
							# Textures are using a lot of code that is lifted from the non PBR extensions
							# consider moving it to a new func? Thid doesn't increase memory usage
							# nor cpu cycles, only increases binary size...
