...
public partial class model_compiler : Node
{
    ...
    Node3D compile_obj_mesh(string path) {
        // Helper functions
            ...
            switch (currentLine[0]) {
                ...
                case "usemtl":
                    if (currentMaterialGroup != "") {
                        ...
                        if (!useNormalData) surfaceTool.GenerateNormals();
                ...
                case "f":
                    if (currentLine.Length <= 4) {
                        for (ushort index = 3; index > 0; index--) {
                            string[] splitIndexes = currentLine[index].Split("/", true);
                            
                            // This should work but using all of these ifs seems sketcky -F

                            if (splitIndexes[2] == "" && useNormalData) {
                                GD.PushWarning("Mesh doesn't specify normal indexes, switching to autogenerated normals");
                                useNormalData = false;
                            } else {
                            ...
                            if (splitIndexes[1] == "" && useUVData) {
                                GD.PushWarning("Mesh doesn't specify UV indexes");
                                useUVData = false;
                            } else {
                                surfaceTool.SetUV(
                            ...
                            if (splitIndexes[0] == "") {
                                GD.PushError("Model corrupted (no vertex data)");
                                _ExitTree();
                            } else {
