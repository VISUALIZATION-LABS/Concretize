...
func _process(_delta: float) -> void:
	...
	if camera.position != self.position:
		...
		X_PLANE.look_at(camera.position, Vector3(1,0,0), false)
		...
		Y_PLANE.look_at(camera.position, Vector3(0,1,0), true)
		...
		Z_PLANE.look_at(camera.position, Vector3(0, 0, 1), false)
